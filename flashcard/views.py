...
def deletar_flashcard(request,id):
    flashcard = Flashcard.objects.filter(user=request.user).filter(id=id)
    if not flashcard:
        messages.add_message(request,constants.ERROR,'Erro ao deletar Flashcard!')
        return redirect(reverse('novo_flashcard'))
...
def iniciar_desafio(request):
    if request.method =='GET':
        ...
        if flashcards.count() < int(qtd_perguntas):
            messages.add_message(request,constants.ERROR,'Flashcards insuficientes!')
...
def listar_desafio(request):
    ...
    return render(request,'listar_desafio.html',{'desafios':desafios,'categorias':categorias,'dificuldades':dificuldades,'status_desafio':status_desafio})
...
def relatorio(request,id):
    ...
    for categoria in categorias:
        ...
        erros_categoria.append(desafio.flashcards.filter(flashcard__categoria=categoria).filter(acertou=False).count())
 
    dados = [acertos,erros]

    dados3 = []
    for indice,dados_array in enumerate(categorias):
        dados3.append({'categoria':dados_array.nome,'acertos':dados2[indice],'erros':erros_categoria[indice]})

    
    lista_ordenada = sorted(dados3, key=lambda x: x['acertos'], reverse=True)
    tres_melhores = lista_ordenada[:3]

    if len(lista_ordenada)>3 and len(lista_ordenada)<=6:
        tres_piores = lista_ordenada[3:6]
    elif len(lista_ordenada)>6:
        tres_piores = lista_ordenada[len(lista_ordenada)-4:len(lista_ordenada)-1]
    
    

    return render(request,'relatorio.html',{'desafio':desafio,'dados':dados,'name_categoria':name_categoria,'dados2':dados2,'categorias':categorias,'tres_melhores':tres_melhores,'tres_piores':tres_piores})
